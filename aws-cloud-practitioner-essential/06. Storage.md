...
connect to only one machine at a time
network drive

root storage for ec2 is also ebs. 8 gib and 30 for windows.
data written there will be lost on terminate.

instance store is also ebs - very fast but data will be lost.

But this ebs is drive for long term storage.
network means latency
not a real physcial drive
can be detached and attached to another ec2.
az specific
billed for provisioned capacity and iops

detach and snapshot is recommended
archive tier is 75 percent cheaper
takes 24 to 72 hours to restore archive

AMI custom
start ec2 from some emi, customize, and stop instance and build the emi (snapsot)
EC2 Image builder
...
bucket name - globally unique name (across all accounts all regions) shared global namespace. account regional namespace - same bucket name across regions
each object has a key
there are no real directories
keys are long name with /

### bucket policies

just like IAM policies - json with allow/deny effect for principals
...
403 - ensure bucket allows public reads
...
99.999999999 - 9 9s durable
99.99 available for s3 standard
...
client side encryption - client encrypts and then uploads
server side encryption - server encrupts after upload

IAM access analyzer

availabilty keeps going down with storage class degradation 

storage fees 128 Kb minimum
retrival fees - per gb

![alt text](s3-costs-etc.png)
...
- Multi zone redundancy (EBS is single az)
- Shared Access
- Elastic Storage - Amazon EFS supports thousands of concurrent NFS connections
- Elastic Storage


Pay per use and no capacity planning
expensive
EFS-IA is upto 92 percent cheaper
...
strategies- 
1. S3
2. pilot light - core fuctions of app ready to scale - minimum setup
3. warm standby - full version of app but minimum size
4. multi-site/hot-site - full version of app and full size

## snowball
edge devices to facilitate data transfer
